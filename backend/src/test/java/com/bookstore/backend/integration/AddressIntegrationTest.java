...
class AddressIntegrationTest {
  ...
  void testCreateAddress_Success() throws Exception {
    ...
    // mock validation
    try (MockedStatic<ValidationUtils> util =
        org.mockito.Mockito.mockStatic(ValidationUtils.class)) {
  ...
  void testGetAddressesByUserId() throws Exception {
    ...
    mockMvc
        .perform(get("/api/address/{userId}", user.getId()))
        .andExpect(status().isOk())
  ...
  void testGetAddressByIdAndUserId() throws Exception {
    ...
    mockMvc
        .perform(get("/api/address/{userId}/{id}", user.getId(), address.getId()))
        .andExpect(status().isOk())
  ...
  void testDeleteAddress_Success() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/address/{id}", address.getId()).with(csrf()))
