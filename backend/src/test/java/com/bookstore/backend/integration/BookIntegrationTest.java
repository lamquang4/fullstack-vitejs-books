...
class BookIntegrationTest {
    ...
    private final File uploadsRoot = new File(System.getProperty("user.dir") + File.separator + "uploads" + File.separator + "books");
    ...
    void beforeEach() throws Exception {
        ...
        // create fixtures
        author = Author.builder().fullname("Integration Author").slug("integration-author").createdAt(LocalDateTime.now()).build();
        author = authorRepository.save(author);

        category = Category.builder().name("Integration Cat").slug("integration-cat").status(1).createdAt(LocalDateTime.now()).build();
        category = categoryRepository.save(category);

        publisher = Publisher.builder().name("Integration Pub").slug("integration-pub").createdAt(LocalDateTime.now()).build();
    ...
    void createBook_withMultipart_shouldReturnOk_and_saveImages() throws Exception {
        ...
        MockMultipartFile bookPart = new MockMultipartFile("book", "", "application/json", jsonBook.getBytes());
        MockMultipartFile imagePart = new MockMultipartFile("files", "cover.jpg", "image/jpeg", "fake-image-bytes".getBytes());
        ...
        File bookDir = new File(System.getProperty("user.dir") + File.separator + "uploads" + File.separator + "books" + File.separator + saved.getId());
    ...
    void getAllBooks_shouldReturnPagingResult() throws Exception {
        ...
        mockMvc.perform(get("/api/book").param("page", "1").param("limit", "12"))
                .andExpect(status().isOk())
    ...
    void getBookBySlugAndById_shouldReturnDetail() throws Exception {
        ...
        mockMvc.perform(get("/api/book/slug/{slug}", "slug-book"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.title").value("Integration Test Book"));

        mockMvc.perform(get("/api/book/{id}", saved.getId()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.title").value("Integration Test Book"));
    }
    ...
    void deleteBook_successAndRemovesFiles() throws Exception {
        ...
        ImageBook imageBook = ImageBook.builder().image("/uploads/books/" + saved.getId() + "/f.jpg").book(saved).createdAt(LocalDateTime.now()).build();
        imageBookRepository.save(imageBook);

        mockMvc.perform(delete("/api/book/{id}", saved.getId()).with(csrf()))
    ...
    void deleteBook_inOrder_shouldFail() throws Exception {
        ...
        User user = User.builder().email("u@t.com").fullname("U").password("p").role(3).status(1).build();
        user = userRepository.save(user);

        Order order = Order.builder().orderCode("ODX").fullname("A").phone("0").speaddress("a").city("c").ward("w").paymethod("COD").status(3).total(100.0).user(user).build();
        order = orderRepository.save(order);

        OrderDetail od = OrderDetail.builder().book(saved).order(order).price(100.0).discount(0.0).quantity(1).build();
    ...
    void updateImagesBook_success() throws Exception {
        ...
        ImageBook img = ImageBook.builder().image("/uploads/books/" + saved.getId() + "/img-old.jpg").book(saved).createdAt(LocalDateTime.now()).build();
        img = imageBookRepository.save(img);

        MockMultipartFile newImage = new MockMultipartFile("files", "img-old.jpg", "image/jpeg", "newcontent".getBytes());

        mockMvc.perform(multipart("/api/book/image")
                        .file(newImage)
    ...
    void deleteImageBook_success() throws Exception {
        ...
        ImageBook img = ImageBook.builder().image("/uploads/books/" + saved.getId() + "/img-del.jpg").book(saved).createdAt(LocalDateTime.now()).build();
        img = imageBookRepository.save(img);

        mockMvc.perform(delete("/api/book/image/{imageId}", img.getId()).with(csrf()))
                .andExpect(status().isNoContent());
