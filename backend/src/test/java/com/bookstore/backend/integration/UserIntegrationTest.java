...
public class UserIntegrationTest {
    ...
    void getUserById_notFound() throws Exception {
        mockMvc.perform(get("/api/user/unknown-id"))
    ...
    void updateUser_success() throws Exception {
        ...
        mockMvc.perform(put("/api/user/{id}", user1.getId())
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(dto)))
                .andExpect(status().isOk())
    ...
    void updateUser_notFound() throws Exception {
        ...
        mockMvc.perform(put("/api/user/unknown-id")
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(dto)))
                .andExpect(status().isNotFound());
    ...
    void updateUserStatus_success() throws Exception {
        mockMvc.perform(patch("/api/user/status/{id}", user1.getId())
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"status\":0}"))
                .andExpect(status().isOk())
    ...
    void updateUserStatus_notFound() throws Exception {
        mockMvc.perform(patch("/api/user/status/{id}", "unknown-id")
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"status\":1}"))
                .andExpect(status().isNotFound());
