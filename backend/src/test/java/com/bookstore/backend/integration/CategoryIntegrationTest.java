...
class CategoryIntegrationTest {
    ...
    void getAllCategories_shouldReturnPaged() throws Exception {
        mockMvc.perform(get("/api/category").param("page", "1").param("limit", "10"))
    ...
    void getCategoryById_found() throws Exception {
        mockMvc.perform(get("/api/category/" + cat1.getId()))
                .andExpect(status().isOk())
    ...
    void getCategoryById_notFound() throws Exception {
        mockMvc.perform(get("/api/category/xxx"))
    ...
    void getActiveCategoriesWithActiveBooks_shouldReturnOnlyActiveOnes() throws Exception {
        ...
        Author author = authorRepository.save(
                Author.builder().fullname("John Writer").slug("john-writer").build()
        );

        Publisher pub = publisherRepository.save(
                Publisher.builder().name("Pub House").slug("pub-house").build()
        );
    ...
    void updateCategory_success() throws Exception {
        ...
        mockMvc.perform(put("/api/category/" + cat1.getId())
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void updateCategory_notFound() throws Exception {
        ...
        mockMvc.perform(put("/api/category/xxx")
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void updateCategoryStatus_shouldHideBooks() throws Exception {
        ...
        Author author = authorRepository.save(
                Author.builder().fullname("A").slug("a").build()
        );
        Publisher pub = publisherRepository.save(
                Publisher.builder().name("P").slug("p").build()
        );
        ...
        mockMvc.perform(patch("/api/category/status/" + cat1.getId())
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void deleteCategory_success() throws Exception {
        ...
        mockMvc.perform(delete("/api/category/" + cat2.getId()).with(csrf()))
    ...
    void deleteCategory_hasBooks_shouldReturn409() throws Exception {
        ...
        mockMvc.perform(delete("/api/category/" + cat1.getId()).with(csrf()))
                .andExpect(status().isConflict());
    }

    @Test
    void deleteCategory_notFound() throws Exception {
        mockMvc.perform(delete("/api/category/xxx").with(csrf()))
                .andExpect(status().isNotFound());
    }
}
