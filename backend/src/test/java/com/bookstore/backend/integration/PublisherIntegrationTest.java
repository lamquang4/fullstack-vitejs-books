...
class PublisherIntegrationTest {
  ...
  void getAllPublishers_shouldReturnPaged() throws Exception {
    ...
    publisherRepository.save(
        Publisher.builder().name("NXB Trẻ").slug("nxb-tre").createdAt(LocalDateTime.now()).build());

    mockMvc
        .perform(get("/api/publisher").param("page", "1").param("limit", "10"))
  ...
  void getPublisherById_shouldReturnPublisher() throws Exception {
    mockMvc
        .perform(get("/api/publisher/{id}", pub1.getId()))
        .andExpect(status().isOk())
  ...
  void createPublisher_shouldCreateSuccessfully() throws Exception {
    ...
    Publisher payload = Publisher.builder().name("Nhà Xuất Bản Giáo Dục").build();
  ...
  void updatePublisher_shouldUpdateSuccessfully() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/publisher/{id}", pub1.getId())
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(payload)))
        .andExpect(status().isOk())
  ...
  void deletePublisher_shouldDeleteSuccessfully() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/publisher/{id}", pub1.getId()).with(csrf()))
