...
class ImageBookRepositoryTest {
    ...
    void findByBook_shouldReturnAllImagesOfBook() {
        imageBookRepository.save(
                ImageBook.builder().book(book).image("a.jpg").build()
        );
        imageBookRepository.save(
                ImageBook.builder().book(book).image("b.jpg").build()
        );

        List<ImageBook> list = imageBookRepository.findByBook(book);

        assertThat(list).hasSize(2);
        assertThat(list).extracting("image")
                .containsExactlyInAnyOrder("a.jpg", "b.jpg");
    }
