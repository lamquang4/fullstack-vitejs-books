...
class OrderRepositoryTest {
    ...
    void setup() {
        ...
        Author author = authorRepository.save(
                Author.builder().fullname("Author1").slug("author1").build()
        );

        Publisher publisher = publisherRepository.save(
                Publisher.builder().name("Pub A").slug("pub-a").build()
        );
    ...
    void findByOrderCodeContainingIgnoreCase_shouldReturnMatchingOrders() {
        ...
        var result = orderRepository.findByOrderCodeContainingIgnoreCase("abc",
    ...
    void findByStatus_shouldReturnCorrectOrders() {
        ...
        var result = orderRepository.findByStatus(3,
                org.springframework.data.domain.PageRequest.of(0, 10));
    ...
    void findByCreatedAtBetween_shouldReturnCorrectOrders() {
        ...
        var result = orderRepository.findByCreatedAtBetween(
                now.minusDays(3), now.plusDays(2),
                org.springframework.data.domain.PageRequest.of(0, 10)
        );
    ...
    void sumTotalByStatusAndCreatedAtBetween_shouldReturnCorrectSum() {
        ...
        Double total = orderRepository.sumTotalByStatusAndCreatedAtBetween(
                3,
                now.minusDays(5),
                now.plusDays(3)
        );
