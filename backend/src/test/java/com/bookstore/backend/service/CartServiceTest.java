...
class CartServiceTest {
  ...
  @BeforeEach
  void setup() {
    user = User.builder().id("u1").email("a@gmail.com").build();
    ...
    item = CartItem.builder().id("ci1").book(book).quantity(3).build();
  ...
  void testGetCartByUserId_NotFound() {
    ...
    assertThrows(EntityNotFoundException.class, () -> cartService.getCartByUserId("u404"));
  }
  ...
  void testAddItemToCart_UserNotFound() {
    ...
    assertThrows(EntityNotFoundException.class, () -> cartService.addItemToCart("u1", "b1", 2));
  }
  ...
  void testAddItemToCart_InvalidQuantity() {
    ...
    assertThrows(IllegalArgumentException.class, () -> cartService.addItemToCart("u1", "b1", 0));
  }
  ...
  void testAddItemToCart_AddNewItem() {
    ...
    when(bookRepository.findById("b1")).thenReturn(Optional.of(book));
    when(cartRepository.findByUserId("u1")).thenReturn(Optional.of(cart));
    when(cartItemRepository.findByCartIdAndBookId("c1", "b1")).thenReturn(Optional.empty());
  ...
  void testAddItemToCart_UpdateExistingItem() {
    ...
    when(bookRepository.findById("b1")).thenReturn(Optional.of(book));
    when(cartRepository.findByUserId("u1")).thenReturn(Optional.of(cart));
    when(cartItemRepository.findByCartIdAndBookId("c1", "b1")).thenReturn(Optional.of(item));
  ...
  void testAddItemToCart_ExceedStock() {
    ...
    item.setQuantity(19);
    assertThrows(IllegalStateException.class, () -> cartService.addItemToCart("u1", "b1", 5));
  }
  ...
  void testUpdateItem_NotFound() {
    ...
    assertThrows(
        EntityNotFoundException.class, () -> cartService.updateCartItemQuantity("ci404", 5));
  }
  ...
  void testRemoveItem_NotFound() {
    when(cartItemRepository.findById("ci404")).thenReturn(Optional.empty());
    assertThrows(EntityNotFoundException.class, () -> cartService.removeItemFromCart("ci404"));
  }
