...
class UserServiceTest {
  ...
  void testGetCustomers_FilterByEmail() {
    Page<User> page = new PageImpl<>(List.of(customer), pageable, 1);
    when(userRepository.findByEmailContainingIgnoreCaseAndRoleIn("cus", List.of(3), pageable))
        .thenReturn(page);
  ...
  void testGetAdmins_NoFilters() {
    ...
    when(userRepository.findByRoleIn(List.of(0, 1, 2), pageable)).thenReturn(page);
  ...
  void testUpdateUserStatus_NotFound() {
    when(userRepository.findById("u1")).thenReturn(Optional.empty());
    assertThrows(EntityNotFoundException.class, () -> userService.updateUserStatus("u1", 0));
  }
  ...
  void testDeleteUser_HasOrders() {
    ...
    assertThrows(IllegalStateException.class, () -> userService.deleteUser("u1"));
  }
  ...
  void testDeleteUser_NotFound() {
    ...
    assertThrows(EntityNotFoundException.class, () -> userService.deleteUser("u1"));
  }
}
