...
class CategoryServiceTest {
  ...
  @BeforeEach
  void setup() {
    category = Category.builder().id("c1").name("Test Category").status(1).build();
  }
  ...
  void testUpdateCategory_Success() {
    Category newData = Category.builder().name("New Name").status(1).build();
  ...
  void testUpdateCategory_DuplicateName() {
    Category duplicate = Category.builder().id("c2").name("Dup").build();

    Category updateData = Category.builder().name("Dup").build();

    when(categoryRepository.findById("c1")).thenReturn(Optional.of(category));
    when(categoryRepository.findByName("Dup")).thenReturn(Optional.of(duplicate));

    assertThrows(
        IllegalArgumentException.class, () -> categoryService.updateCategory("c1", updateData));
  }
  ...
  void testUpdateCategory_HiddenCategory_HidesBooks() {
    ...
    Category update = Category.builder().name("Hidden Cat").status(0).build();
    ...
    when(bookRepository.findByCategoryAndStatus(category, 1)).thenReturn(List.of(b1, b2));
    when(categoryRepository.save(any())).thenReturn(category);
  ...
  void testUpdateCategoryStatus_HideBooks() {
    ...
    when(categoryRepository.findById("c1")).thenReturn(Optional.of(category));
    when(bookRepository.findByCategoryAndStatus(category, 1)).thenReturn(List.of(b));
    when(categoryRepository.save(any())).thenReturn(category);
  ...
  void testUpdateCategoryStatus_NotFound() {
    when(categoryRepository.findById("c1")).thenReturn(Optional.empty());
    assertThrows(
        EntityNotFoundException.class, () -> categoryService.updateCategoryStatus("c1", 1));
  }
  ...
  void testDeleteCategory_NotFound() {
    ...
    assertThrows(EntityNotFoundException.class, () -> categoryService.deleteCategory("c1"));
  }
  ...
  void testDeleteCategory_HasBooks() {
    ...
    assertThrows(IllegalStateException.class, () -> categoryService.deleteCategory("c1"));
  }
}
