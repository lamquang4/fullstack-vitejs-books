...
class PublisherControllerTest {
  ...
  void getAllPublishers_shouldReturnPage() throws Exception {
    ...
    Mockito.when(publisherService.getAllPublishers(anyInt(), anyInt(), anyString()))
        .thenReturn(page);

    mockMvc
        .perform(get("/api/publisher").param("page", "1").param("limit", "12").param("q", "test"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.publishers[0].id").value("pub1"))
        .andExpect(jsonPath("$.publishers[0].name").value("Test Publisher"))
  ...
  void getAllPublishers1_shouldReturnList() throws Exception {
    ...
    Mockito.when(publisherService.getAllPublishers1()).thenReturn(List.of(mockPublisher()));

    mockMvc
        .perform(get("/api/publisher/all"))
        .andExpect(status().isOk())
  ...
  void getPublisherById_found() throws Exception {
    ...
    mockMvc
        .perform(get("/api/publisher/{id}", "pub1"))
        .andExpect(status().isOk())
  ...
  void getPublisherById_notFound() throws Exception {
    ...
    Mockito.when(publisherService.getPublisherById("404")).thenReturn(Optional.empty());

    mockMvc.perform(get("/api/publisher/{id}", "404")).andExpect(status().isNotFound());
  }
  ...
  void createPublisher_shouldReturnPublisher() throws Exception {
    ...
    Mockito.when(publisherService.createPublisher(any())).thenReturn(publisher);
  ...
  void updatePublisher_shouldUpdate() throws Exception {
    ...
    Mockito.when(publisherService.updatePublisher(eq("pub1"), any())).thenReturn(updated);

    mockMvc
        .perform(
            put("/api/publisher/{id}", "pub1")
                .with(csrf())
                .contentType("application/json")
  ...
  void updatePublisher_notFound() throws Exception {
    ...
    Mockito.when(publisherService.updatePublisher(eq("404"), any())).thenReturn(null);

    mockMvc
        .perform(
            put("/api/publisher/{id}", "404")
                .with(csrf())
                .contentType("application/json")
  ...
  void deletePublisher_shouldReturn204() throws Exception {
    ...
    Mockito.doNothing().when(publisherService).deletePublisher("pub1");

    mockMvc
        .perform(delete("/api/publisher/{id}", "pub1").with(csrf()))
