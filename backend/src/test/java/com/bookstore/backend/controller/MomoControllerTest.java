...
class MomoControllerTest {
  ...
  void payWithMomo_success() throws Exception {
    ...
    MomoResponse response =
        MomoResponse.builder()
            .payUrl("https://momo.test/pay")
            .qrCodeUrl("https://momo.test/qr.png")
            .resultCode(0)
            .build();

    Mockito.when(orderService.getOrderByOrderCode("ORD123")).thenReturn(order);

    Mockito.when(momoService.createPayment(order)).thenReturn(response);

    mockMvc
        .perform(post("/api/payment/momo/qr/{orderCode}", "ORD123").with(csrf()))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.payUrl").value("https://momo.test/pay"))
  ...
  void redirect_success_flow() throws Exception {
    ...
    Mockito.when(momoService.handleSuccessfulPayment(Mockito.any())).thenReturn(true);
  ...
  void redirect_fail_flow() throws Exception {
    ...
    Mockito.when(momoService.handleSuccessfulPayment(Mockito.any())).thenReturn(false);

    mockMvc
        .perform(
            get("/api/payment/momo/redirect").param("resultCode", "0").param("orderId", "ORD404"))
        .andExpect(status().isFound())
        .andExpect(header().string("Location", "http://localhost:5173/order-result?result=fail"));
  }
  ...
  void redirect_cancel_flow() throws Exception {
    ...
    mockMvc
        .perform(get("/api/payment/momo/redirect").param("resultCode", "1"))
        .andExpect(status().isFound())
