...
class AddressControllerTest {
  ...
  void getAddressesByUserId_shouldReturnList() throws Exception {
    ...
    Mockito.when(addressService.getAddressesByUserId(USER_ID)).thenReturn(List.of(address));

    mockMvc
        .perform(get("/api/address/{userId}", USER_ID))
  ...
  void getAddressByIdAndUserId_shouldReturnAddress() throws Exception {
    ...
    Mockito.when(addressService.getAddressByIdAndUserId(ADDRESS_ID, USER_ID)).thenReturn(address);

    mockMvc
        .perform(get("/api/address/{userId}/{id}", USER_ID, ADDRESS_ID))
        .andExpect(status().isOk())
  ...
  void createAddress_shouldReturnCreatedAddress() throws Exception {
    ...
    Mockito.when(addressService.createAddress(Mockito.any(AddressDTO.class))).thenReturn(address);
  ...
  void updateAddress_shouldReturnUpdatedAddress() throws Exception {
    ...
    Mockito.when(
            addressService.updateAddress(Mockito.eq(ADDRESS_ID), Mockito.any(AddressDTO.class)))
        .thenReturn(address);

    mockMvc
        .perform(
            put("/api/address/{id}", ADDRESS_ID)
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(dto)))
        .andExpect(status().isOk())
  ...
  void deleteAddress_shouldReturnNoContent() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/address/{id}", ADDRESS_ID).with(csrf()))
