...
public class BookService {
  ...
  public Page<BookDTO> getAllActiveBooks(
      int page, String q, String sort, Integer min, Integer max) {
    int limit = 12;
    int status = 1;
    Pageable pageable = PageRequest.of(page - 1, limit, Sort.by("createdAt").descending());
    ...
    if (q != null && !q.isEmpty()) {
      bookPage =
          bookRepository.searchByTitleAuthorPublisherCategoryAndPrice(
              q, status, min, max, pageable);
    } else {
      if (sort == null || sort.isEmpty()) {
        ...
        switch (sort) {
          case "price-asc":
            bookPage =
                bookRepository.findByStatusAndPriceRangeOrderByEffectivePriceAsc(
                    status, min, max, pageable);
            break;
          case "price-desc":
            bookPage =
                bookRepository.findByStatusAndPriceRangeOrderByEffectivePriceDesc(
                    status, min, max, pageable);
            break;
          case "bestseller":
            bookPage =
                bookRepository.findByStatusAndPriceRangeOrderByTotalSold(
                    status, min, max, pageable);
            break;
          default:
            bookPage = bookRepository.findByStatusAndPriceRange(status, min, max, pageable);
  ...
  public Page<BookDTO> getDiscountedActiveBooks(
      int page, String q, String sort, Integer min, Integer max) {
    int limit = 12;
    int status = 1;
    Pageable pageable = PageRequest.of(page - 1, limit, Sort.by("createdAt").descending());
    ...
    if (q != null && !q.isEmpty()) {
      bookPage =
          bookRepository.searchDiscountedActiveCategoryAndPrice(q, status, min, max, pageable);
    } else {
      if (sort == null || sort.isEmpty()) {
        ...
        switch (sort) {
          ...
          case "price-desc":
            bookPage =
                bookRepository.findDiscountedAndPriceRangeOrderByEffectivePriceDesc(
                    status, min, max, pageable);
            break;
          case "bestseller":
            bookPage =
                bookRepository.findDiscountedAndPriceRangeOrderByTotalSold(
                    status, min, max, pageable);
            break;
          default:
            bookPage = bookRepository.findDiscountedAndPriceRange(status, min, max, pageable);
  ...
  public Page<BookDTO> getActiveBooksByCategory(
      String slug, int page, String q, String sort, Integer min, Integer max) {
    int limit = 12;
    int status = 1;
    Pageable pageable = PageRequest.of(page - 1, limit, Sort.by("createdAt").descending());
    ...
    if (q != null && !q.isEmpty()) {
      bookPage =
          bookRepository.searchByCategoryAndTitleAuthorPublisherCategoryAndPrice(
              slug, q, status, min, max, pageable);
    } else {
      if (sort == null || sort.isEmpty()) {
        bookPage =
            bookRepository.findByCategorySlugAndStatusAndPriceRange(
                slug, status, min, max, pageable);
      } else {
        switch (sort) {
          case "price-asc":
            bookPage =
                bookRepository.findByCategorySlugAndStatusAndPriceRangeOrderByEffectivePriceAsc(
                    slug, status, min, max, pageable);
            break;
          case "price-desc":
            bookPage =
                bookRepository.findByCategorySlugAndStatusAndPriceRangeOrderByEffectivePriceDesc(
                    slug, status, min, max, pageable);
            break;
          case "bestseller":
            bookPage =
                bookRepository.findByCategorySlugAndStatusAndPriceRangeOrderByTotalSold(
                    slug, status, min, max, pageable);
            break;
          default:
            bookPage =
  ...
  public BookDetailDTO getBookBySlug(String slug) {
    ...
    if (book.getAuthor() != null) {
      authorDTO =
          new AuthorDTO(
              book.getAuthor().getId(), book.getAuthor().getFullname(), book.getAuthor().getSlug());
    }
    ...
    List<ImageBookDTO> imageDTOs =
        (book.getImages() != null && !book.getImages().isEmpty())
            ? book.getImages().stream()
        ...
        try {
          ...
          if (contentType == null || !allowedMimeTypes.contains(contentType)) {
            throw new IllegalArgumentException(
                "Chỉ cho phép hình JPG, PNG hoặc WEBP " + contentType);
          }
          ...
          ImageBook imageBook = ImageBook.builder().image(relativePath).book(savedBook).build();
    ...
    if (files != null && !files.isEmpty()) {
      ...
      for (MultipartFile file : files) {
        try {
          ...
          ImageBook imageBook = ImageBook.builder().image(relativePath).book(updatedBook).build();
              ...
              if (status == 1) {
                if (book.getCategory().getStatus() == 0) {
                  throw new IllegalStateException(
                      "Không thể hiện sách vì danh mục của sách đang bị ẩn");
                }
              }
  ...
  public void deleteBook(String id) {
    ...
    if (cartItemRepository.existsByBook(book)) {
      throw new IllegalStateException(
          "Sách này không thể bị xóa vì đang có trong giỏ hàng của người dùng");
    }
