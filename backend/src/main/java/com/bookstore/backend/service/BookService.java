...
public class BookService {
    ...
    public BookService(BookRepository bookRepository, ImageBookRepository imageBookRepository,
            OrderDetailRepository orderDetailRepository, CartItemRepository cartItemRepository,
    ...
    public Page<BookDTO> getAllActiveBooks(int page, String q, String sort, Integer min, Integer max) {
        ...
        if (q != null && !q.isEmpty()) {
            ...
            if (sort == null || sort.isEmpty()) {
                ...
                switch (sort) {
                    case "price-asc":
                        bookPage = bookRepository.findByStatusAndPriceRangeOrderByEffectivePriceAsc(status, min, max,
                                pageable);
                        break;
                    case "price-desc":
                        bookPage = bookRepository.findByStatusAndPriceRangeOrderByEffectivePriceDesc(status, min, max,
                                pageable);
                        break;
                    case "bestseller":
    ...
    public Page<BookDTO> getDiscountedActiveBooks(int page, String q, String sort, Integer min, Integer max) {
        ...
        if (q != null && !q.isEmpty()) {
            ...
            if (sort == null || sort.isEmpty()) {
                ...
                switch (sort) {
                    case "price-asc":
                        bookPage = bookRepository.findDiscountedAndPriceRangeOrderByEffectivePriceAsc(status, min, max,
                                pageable);
                        break;
                    case "price-desc":
                        bookPage = bookRepository.findDiscountedAndPriceRangeOrderByEffectivePriceDesc(status, min, max,
                                pageable);
                        break;
                    case "bestseller":
                        bookPage = bookRepository.findDiscountedAndPriceRangeOrderByTotalSold(status, min, max,
                                pageable);
                        break;
                    default:
                        bookPage = bookRepository.findDiscountedAndPriceRange(status, min, max, pageable);
    ...
    public Page<BookDTO> getActiveBooksByCategory(String slug, int page, String q, String sort, Integer min,
            Integer max) {
        int limit = 12;
        int status = 1;
        Pageable pageable = PageRequest.of(page - 1, limit, Sort.by("createdAt").descending());
        ...
        if (q != null && !q.isEmpty()) {
            bookPage = bookRepository.searchByCategoryAndTitleAuthorPublisherCategoryAndPrice(slug, q, status, min, max,
                    pageable);
        } else {
            if (sort == null || sort.isEmpty()) {
                ...
                switch (sort) {
                    case "price-asc":
                        bookPage = bookRepository.findByCategorySlugAndStatusAndPriceRangeOrderByEffectivePriceAsc(slug,
                                status, min, max, pageable);
                        break;
                    case "price-desc":
                        bookPage = bookRepository.findByCategorySlugAndStatusAndPriceRangeOrderByEffectivePriceDesc(
                                slug, status, min, max, pageable);
                        break;
                    case "bestseller":
                        bookPage = bookRepository.findByCategorySlugAndStatusAndPriceRangeOrderByTotalSold(slug, status,
                                min, max, pageable);
                        break;
                    default:
                        bookPage = bookRepository.findByCategorySlugAndStatusAndPriceRange(slug, status, min, max,
                                pageable);
                }
            }
        }
    ...
    public Book createBook(Book book, List<MultipartFile> files) {
        ...
        if (files != null && !files.isEmpty()) {
            String uploadRoot = System.getProperty("user.dir") + File.separator + "uploads" + File.separator + "books"
                    + File.separator + id;
