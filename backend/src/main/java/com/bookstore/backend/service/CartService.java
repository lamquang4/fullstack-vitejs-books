  ...
  public void addItemToCart(String userId, String bookId, int quantity) {
    ...
    Cart cart =
        cartRepository
            ...
            .orElseGet(
                () -> {
                  Cart newCart = Cart.builder().user(user).createdAt(LocalDateTime.now()).build();
    ...
    if (existingItemOpt.isPresent()) {
      ...
      CartItem newItem = CartItem.builder().cart(cart).book(book).quantity(finalQuantity).build();
  ...
  public void updateCartItemQuantity(String cartItemId, int quantity) {
    CartItem item =
        cartItemRepository
            .findById(cartItemId)
            .orElseThrow(
                () -> new EntityNotFoundException("Sản phẩm trong giỏ hàng không tìm thấy"));
