...
public interface OrderRepository extends JpaRepository<Order, String> {
  ...
  Page<Order> findByStatusAndCreatedAtBetween(
      Integer status, LocalDateTime start, LocalDateTime end, Pageable pageable);

  Page<Order> findByOrderCodeContainingIgnoreCaseAndCreatedAtBetween(
      String orderCode, LocalDateTime start, LocalDateTime end, Pageable pageable);

  Page<Order> findByOrderCodeContainingIgnoreCaseAndStatusAndCreatedAtBetween(
      String orderCode, Integer status, LocalDateTime start, LocalDateTime end, Pageable pageable);
  ...
  @Query(
      "SELECT SUM(o.total) FROM Order o WHERE o.status = :status AND o.createdAt BETWEEN :start AND :end")
  Double sumTotalByStatusAndCreatedAtBetween(
  ...
  @Query(
      "SELECT SUM(od.quantity) FROM OrderDetail od WHERE od.order.status = :status AND od.order.createdAt BETWEEN :start AND :end")
  Long sumQuantityByStatusAndCreatedAtBetween(
      @Param("status") Integer status,
      @Param("start") LocalDateTime start,
      @Param("end") LocalDateTime end);
}
